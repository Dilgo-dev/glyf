...
//! unlockpt + ptsname_r. Spawns a child via fork + setsid + TIOCSCTTY
//! + execvp. Callers read and write on the controller fd.
...
comptime {
    if (builtin.os.tag == .windows) {
        @compileError("pty_posix module is posix-only");
    }
}
...
pub const Pty = struct {
    ...
    /// Read bytes produced by the child on the controller fd.
    pub fn read(self: Pty, buf: []u8) Error!usize {
        return posix.read(self.controller, buf) catch Error.ReadFailed;
    }

    /// Write bytes to the child via the controller fd.
    pub fn write(self: Pty, buf: []const u8) Error!usize {
        return posix.write(self.controller, buf) catch Error.WriteFailed;
    }

    /// Spawn a child attached to the follower as its controlling
    /// terminal. argv[0] is the program name, resolved against PATH.
    /// On success the parent-side follower fd is closed.
    pub fn spawn(
        self: *Pty,
        ...
        const argv_z = a.alloc(?[*:0]const u8, argv.len + 1) catch return Error.OutOfMemory;
        for (argv, 0..) |arg, i| {
            const z = a.dupeZ(u8, arg) catch return Error.OutOfMemory;
            argv_z[i] = z.ptr;
        }
        argv_z[argv.len] = null;

        const file: [*:0]const u8 = argv_z[0].?;
...
test "spawn echo reads output" {
    ...
    const argv = [_][]const u8{ "echo", "hello-pty" };
